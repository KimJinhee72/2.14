  ...
  <div
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <div class="space-y-4">
          <div
          v-for="reservation in recentReservations" :key="reservation.id"
...
const filteredReservations = computed(() => {
  ...
  // 날짜를 필터링: 시작일과 종룍일을 지정한 경우, 해당 범위내의 예약난 필터링
  if (dateRange.value.start && dateRange.value.end) {
    result = result.filter((reservation) => {
      // reservations의 date항목을 담음
      const reservationDate = new Date(reservation.date);
      const startDate = new Date(dateRange.value.start); //시작일
      const endtDate = new Date(dateRange.value.end); //종료일
      // 리턴값 설정
      return reservationDate >= startDate && reservationDate <= endtDate;
    });
  }
  // 청소 유형 필터링

          ...
          <div class="space-y-6">
            ...
            <div class="flex items-center space-x-4">
              ...
              <div>
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                  <span
                    v-for="tag in seletedCustomer.tags"
                    :key="tag.id"
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                ...
                <select
                  v-model="seletedCustomer.status"
                  class="w-full bg-transparent border-b border-gray-300 dark:border-gray-600 focus:border-indigo-500 focus:ring-0 text-gray-900 dark:text-white">
                  <option value="active">활성</option>
                  <option value="inactive">비활성</option>
                  <option value="blocked">차단</option>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p
                    ...
                    {{ seletedCustomer.reservationCount }}건
                  </p>
                </div>
                <div>
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                  ...
                  <p
                    class="text-lg font-semibold text-gray-900 dark:text-white">
                    {{ formatCurrency(seletedCustomer.totalSpent) }}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
              ...
              <div class="flex flex-wrap gap-2">
                <div
                  v-for="(item, index) in seletedCustomer.favoriteItems"
                  :key="index"
                  class="flex items-center gap-2">
                  <input
                    v-model="seletedCustomer.favoriteItems[index]"
                    type="text"
...
//고객 상세 보기 모달 열기
const seletedCustomer = ref("null");
const showCustomerDetails = (customer) => {
  seletedCustomer.value = customer; //선택된 고객정보 저장
  showCustomerDetailsModal.value = true;
};
// 고객 상세 보기 모달 닫기
const closeCustomerDetailsModal = () => {
  showCustomerDetailsModal.value = false;
  seletedCustomer.value = null;
};
// 선호메뉴취소
const removeFavoriteItem = (index) => {
  // x눌러 취소 하겠다는 의미
  seletedCustomer.value.favoriteItems.splice(index, 1);
};
// 선호 항목 추가
const addFavoriteItem = () => {
  if (!seletedCustomer.value.favoriteItems) {
    seletedCustomer.value.favoriteItems = []; //배열이 없으면 초기화
  }
  seletedCustomer.value.favoriteItems.push("");
};
// 모달 저장
const saveCustomerDetails = () => {
  // 1.고객 목록에서 현재 선택된 고객의 인덱스 찾기
  const index = customers.value.findIndex(
    (c) => c.id === seletedCustomer.value.id
  );
  if (index !== -1) {
    // 2.빈 문자열로 된 선호 메뉴 항목은 제거(trim 빈것 제거)
    seletedCustomer.value.favoriteItems =
      seletedCustomer.value.favoriteItems.filter((item) => item.trim() !== "");
    // 3.수정된 고객 정보를 고객 목록에 반영
    customers.value[index] = { ...seletedCustomer.value };
    // 4.모달닫기
...
const confirmDelete = (customer) => {
  customerToDelete.value = customer; //삭제할 고객 정보를 저장
  showDeleteModal.value = true; //삭제 확인 모달 표시
};
// 삭제 모달 닫기
const closeDeleteModal = () => {
  closeDeleteModal.value = null;
...
// 맨위 전체 고객 수를 계산하는 계산 속성
const totalCustomers = computed(() => customers.value.length);
// 활성상태인 고객만 필터링하여 그 수를 계산하는 계산 속성

  ...
  <div class="space-y-4 md:space-y-6 p-4 md:p-6">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <!-- getStatusClass도 해당 상태에따라 변화하게 적용  -->
                <span
                  :class="[
                    getStatusClass(job.status),
              ...
              <td class="px-6 py-4">
                ...
                <div class="text-sm text-gray-500">
                  <!-- 전화번호 -->
                  <!-- ?.는 옵셔널 체이닝(Optional Chaining) 객체의 속성(property)이나 메서드에 접근할 때 중간 값이 null 또는 undefined일 경우 에러 없이 안전하게 무시하고 undefined를 반환 -->
                  <i class="fas fa-phone mr-1"></i
                  >{{ job.contact?.primary || job.phone }}
                </div>
              ...
              <td class="px-6 py-4">
                <!-- 장소 타입 -->
                <div class="text-sm text-gray-900">
                  {{ job.location?.type || "정보없음" }}
                </div>
                <!-- 층수 -->
                <div class="text-sm text-gray-500">
                  {{ job.location?.floor || "정보없음" }}
                </div>
                <!-- 주소 -->
                <div class="text-sm text-gray-500 mt-1">
                  <i class="fas fa-map-marker-alt mr-1"></i>{{ job.address }}
    ...
    <div
      ...
      <div class="flex gap-2">
        ...
        <!-- :disabled=" currentPage === totalPages" 2번이 담긴것으로 설정 -->
        <button
          @click="nextPage"
...
const filteredJobs = computed(() => {
  // assignedJobs.value 배열을 목사해서 result에 담음 (검색)
  let result = [...assignedJobs.value];
  // 검색필터링 내용
  if (searchQuery.value) {
    // 입력된 검색을 소문자로 변환해 query 에 담음
    const query = searchQuery.value.toLowerCase();
    // 다시 재할당  job에 결과들이 담김
    result = result.filter((job) => {
      console.log(job);
      return (
        // 고객명 필터를 소문자로 입력값을 포함하라
        job.customer.toLowerCase().includes(query) ||
        // 주소 필터를 소문자로 입력값을 포함하라
...
const totalPages = computed(() => {
  // 전체 필터된 filteredJobs(10)를 5로 나누니 2페이지
  return Math.ceil(filteredJobs.value.length / itemsPerPage.value);
});
// 페이지네이션 적용
const paginatedJobs = computed(() => {
  // 현재 페이지에서 시작할 인텍스(0) start를 설정
  const start = (currentPage.value - 1) * itemsPerPage.value;
  // 끝 인텍스( start + 한펜이지 표시할 수)
...
const startJob = (job) => {
  ...
  // 같은 id를 가진 작업이 배열에 실제로 존재하는 경우
  if (index !== -1) {
    assignedJobs.value[index] = {
      ...job,
      status: "in_progress",
      startTime: new Date().toISOString(), //시작시간설정
    };
  }
};
// 완료 클릭
const completeJob = (job) => {
  ...
  if (index !== -1) {
    assignedJobs.value[index] = {
      ...job,
      status: "completed",
      completeTime: new Date().toISOString(), //시작시간 기록
    };
  }
  // 상세 정보에 완료버튼을 누르면 모달닫기
  //   사용자가 이미선택한 작업을 다시 완료 클릭한 경우
  if (selectedJob.value && selectedJob.value.id === job.id) {
    closeModal(); //모달을 닫아서 선택을 취소 , 중복선택을 방지
  }
};
// 상세모달
const selectedJob = ref(null); //개발자가 처음에 아직 아무것도 선택안된 상태 설정
const showJobDetails = (job) => {
  //job은 선택상세임
  selectedJob.value = { ...job }; //누르면
};
...
// 닫기 버튼
const closeModal = () => {
  selectedJob.value = null; //초기상태로 다시 변환
};
</script>
<style scoped></style>

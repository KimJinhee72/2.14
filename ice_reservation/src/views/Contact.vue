        ...
        <form class="contact-form" @submit.prevent="submitInquiry">
          ...
          <div class="form-group">
            <label for="file-label">
              <input @change="handleFileUpload" type="file" accept="image/*, .pdf" />
...
function submitInquiry() {
    showConfirmModal.value = true; //모달창 띄워줄예정
}
function getCategoryName(category){//선택 영어 value 값을 한글로 변환해 모달창서 한글로 보여짐
    const categoryMap = {
        reservation: "예약 관련",
        service: "서비스 관련",
        payment: "결제 관련",
...
function handleFileUpload(event){
    const file = event.target.files[0]//선택한 파일로 할당
    if(file){
...
function confirmSubmit(){
    alert("문의가 접수 되었습니다. 빠른 시일내에 답변 드리겠습니다.")
    router.push("/")
}
</script>
...
.contact-container {
  display: grid;
  grid-template-columns: 1fr 2fr; /*1:2 비율을 의미함*/
}
.contact-info {
  background: #f8f9fa;
  padding: 1.5rem;
